...
// Membersihkan URL YouTube dari Parameter Pelacak / Sampah
...
// METODE FETCH DENGAN RETRY & TIMEOUT OTOMATIS
...
// PROSES PENGAMBILAN METADATA UTAMA (Membaca Root JSON API Ryzumi)
async function fetchMetadata() {
    ...
    try {
        ...
        // PEMETAAN PRESISI ROOT JSON SIFAT API RYZUMI
        if (data && data.title) {
            analyzedMetadata = {
                title: data.title,
                ...
                videoDirectUrl: data.url, // Memetakan direct URL (.url) dari JSON Root
                originalLink: cleanedUrl,
                qualityGivenByAPI: data.quality || quality, // Menyimpan kualitas asli yang diberikan oleh API
        ...
        let errorMsg = currentLang === 'id' ? "Gagal memproses API. Membuka Mode Cadangan." : "API failed. Fallback active.";
        if (err.message === "RATE_LIMIT") {
            errorMsg = currentLang === 'id' ? "IP Anda dibatasi sementara oleh server API." : "Your IP is rate-limited by the API server.";
        } else if (err.message === "SERVER_OVERLOAD") {
            errorMsg = currentLang === 'id' ? "Server API sedang kelebihan beban (Overload)." : "API Server is overloaded.";
        } else if (err.message === "TIMEOUT") {
            errorMsg = currentLang === 'id' ? "Waktu respon habis (Timeout) karena jaringan lambat." : "Request timed out.";
        }
        
        showToast(errorMsg, "error");
        activateMockupFallback(cleanedUrl); 
    }
}

// OTO-SINKRONISASI KUALITAS SAAT DROPDOWN DIUBAH (LIVE-SYNC ENGINE)
async function syncQualityOnDropdownChange() {
    ...
    // Kunci tombol video & tampilkan status loading demi keamanan request
    dlBtn.disabled = true;
    dlBtn.innerHTML = `<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Syncing...`;
    lucide.createIcons();
    ...
    try {
        ...
        if (data && data.url) {
            // Perbarui URL download video dan perbarui badge kualitas server secara real-time
        ...
        showToast(currentLang === 'id' ? "Resolusi tidak tersedia. Memakai cadangan otomatis." : "Resolution unavailable. Keeping auto-best option.", "error");
    } finally {
...
function updateUIData() {
    ...
    // Tampilkan label kualitas server yang sebenarnya dikirim API
    document.getElementById('actual-quality').innerText = analyzedMetadata.qualityGivenByAPI;
...
async function startDownload(type) {
    ...
    if (type === 'video') {
        ...
        try {
            ...
            // Langsung panggil JSON API ytmp3 dan ambil .url dari root
            const data = await fetchWithRetryAndTimeout(apiEndpoint, { headers: { 'accept': 'application/json' } });
            ...
            // Simulasi link dummy jika API MP3 sedang sibuk/down
            directDownloadUrl = `https://ytdlpyton.nvlgroup.my.id/download/file/Game%20Minecraft%20Punya%20Sejarah%20Unik%E2%80%BC%EF%B8%8F%23shorts_128k.mp3`;
            runSimulatedProgress(directDownloadUrl, type);
        }
    }
}
...
function runSimulatedProgress(downloadUrl, type) {
    ...
    const interval = setInterval(() => {
        ...
        if (currentVal >= 100) {
            ...
            // Kirim link unduhan langsung ke browser
