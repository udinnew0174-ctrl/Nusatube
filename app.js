/**
 * NusaTube Downloader - Application Logic File (Optimized & Secure)
 */
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Bind UI Switch Toggle
    document.getElementById('sound-toggle').checked = soundEnabled;
    document.getElementById('haptic-toggle').checked = hapticEnabled;
    
    // Bind Event Listener untuk Auto-Sync Kualitas
    document.getElementById('quality-selector').addEventListener('change', syncQualityOnDropdownChange);
...
// Synthesizer Audio & Haptic System (Pure Web Audio)
function triggerFeedback() {
    if (hapticEnabled && navigator.vibrate) {
        navigator.vibrate(12); // Vibrasi haptic ponsel
    }
    if (soundEnabled) {
        try {
...
function switchTab(tabId) {
    ...
    // Desktop Nav Active Tab Styling
...
// FUNGSI UTAMA: Membersihkan URL YouTube dari Parameter Sampah Pelacak
function sanitizeYoutubeUrl(url) {
    if (!url) return "";
    
    // Pola regex untuk menangkap ID Video YouTube (11 karakter)
    const regex = /(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/||user\/[^\/]+\/|embed\/|watch\?(?:.*&)?v=|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/;
    ...
    return url; // Kembalikan url asal jika tidak cocok dengan struktur regex
}
...
async function pasteFromClipboard() {
    ...
    try {
        ...
        if (txt) {
            ...
            validateUrlInput();
            showToast(currentLang === 'id' ? "Link disalin dari clipboard!" : "Pasted from clipboard!", "success");
        }
    } catch {
...
// METODE FETCH KUAT DENGAN BATAS WAKTU & RETRY OTOMATIS (RESILIENCE WRAPPER)
async function fetchWithRetryAndTimeout(url, options = {}, retries = CONFIG.API.MAX_RETRIES, timeout = CONFIG.API.TIMEOUT_MS) {
    for (let i = 0; i <= retries; i++) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status >= 500) throw new Error("SERVER_OVERLOAD");
                throw new Error(`HTTP_${response.status}`);
            }
            return await response.json(); // Berhasil mengembalikan data JSON
        } catch (err) {
            clearTimeout(timeoutId);
            ...
            // Tunggu 1,5 detik sebelum mengirim request ulang (Exponential Backoff Delay)
            await new Promise(resolve => setTimeout(resolve, 1500));
        }
    }
}

// PROSES PENGAMBILAN METADATA API
async function fetchMetadata() {
    triggerFeedback();
    let rawUrl = document.getElementById('youtube-url').value.trim();
    const quality = document.getElementById('quality-selector').value;

    if (!rawUrl || !validateYoutubeUrl(rawUrl)) {
        showToast(currentLang === 'id' ? "Format link tidak valid!" : "Invalid YouTube link!", "error");
        return;
    }

    // Bersihkan link YouTube dari parameter pelacak
    const cleanedUrl = sanitizeYoutubeUrl(rawUrl);
    ...
    try {
        const apiEndpoint = `${CONFIG.API.VIDEO_ENDPOINT}?url=${encodeURIComponent(cleanedUrl)}&quality=${quality}`;
        
        // Panggil fetch kokoh
        const data = await fetchWithRetryAndTimeout(apiEndpoint, { headers: { 'accept': 'application/json' } });
        ...
        if (data && data.title) {
            analyzedMetadata = {
                ...
                videoDirectUrl: data.url, // Membaca file unduhan video instan dari API
                originalLink: cleanedUrl,
                qualityGivenByAPI: data.quality || quality, // Resolusi asli server
                isShorts: cleanedUrl.includes('/shorts/')
            };
        ...
        // Pemetaan pesan error secara transparan kepada pengguna
        let errorMsg = currentLang === 'id' ? "Gagal memproses API. Membuka Mode Cadangan." : "API processing failed. Fallback active.";
        if (err.message === "RATE_LIMIT") {
            errorMsg = currentLang === 'id' ? "Limit request penuh! IP Anda dibatasi sementara oleh server API." : "Too many requests. Your IP is rate-limited by the API server.";
        } else if (err.message === "SERVER_OVERLOAD") {
            errorMsg = currentLang === 'id' ? "Server API sedang kelebihan beban beban (Overload). Silakan coba lagi." : "API Server is overloaded. Please try again shortly.";
        } else if (err.message === "TIMEOUT") {
            errorMsg = currentLang === 'id' ? "Request waktu habis (Timeout) karena jaringan buruk." : "Request timed out due to slow network connection.";
        }
        
        showToast(errorMsg, "error");
        activateMockupFallback(cleanedUrl); // Menjalankan simulasi agar UI tidak hang/blank
    }
}

// SYNC KUALITAS DINAMIS SAAT DROPDOWN DIUBAH (LIVE-SYNC ENGINE)
async function syncQualityOnDropdownChange() {
    if (!analyzedMetadata) return;

    triggerFeedback();
    const newQuality = document.getElementById('quality-selector').value;
    const dlBtn = document.getElementById('btn-dl-video');
    const originalText = dlBtn.innerHTML;

    dlBtn.disabled = true;
    dlBtn.innerHTML = `<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Syncing ${newQuality}...`;
    lucide.createIcons();

    try {
        const apiEndpoint = `${CONFIG.API.VIDEO_ENDPOINT}?url=${encodeURIComponent(analyzedMetadata.originalLink)}&quality=${newQuality}`;
        const data = await fetchWithRetryAndTimeout(apiEndpoint, { headers: { 'accept': 'application/json' } });
        
        if (data && data.url) {
            analyzedMetadata.videoDirectUrl = data.url;
            analyzedMetadata.qualityGivenByAPI = data.quality || newQuality;
            
            document.getElementById('actual-quality').innerText = analyzedMetadata.qualityGivenByAPI;
            
            showToast(currentLang === 'id' ? `Kualitas dialihkan ke: ${analyzedMetadata.qualityGivenByAPI}` : `Quality switched to: ${analyzedMetadata.qualityGivenByAPI}`, "success");
        } else {
            throw new Error();
        }
    } catch (err) {
        showToast(currentLang === 'id' ? "API gagal merespons kualitas tersebut. Memakai kualitas terbaik otomatis." : "API did not respond for selected quality. Keeping auto-best option.", "error");
    } finally {
...
function updateUIData() {
    ...
    // Tampilkan label kualitas nyata dari API
    document.getElementById('actual-quality').innerText = analyzedMetadata.qualityGivenByAPI;
...
// Fallback Mockup jika API Utama Limit/Off
function activateMockupFallback(url) {
    ...
    analyzedMetadata = {
        ...
        originalLink: url,
        qualityGivenByAPI: "360x640p (Shorts Limit)",
        isShorts: true
    };
...
async function startDownload(type) {
    ...
    if (type === 'video') {
        ...
        lucide.createIcons();
        try {
            const apiEndpoint = `${CONFIG.API.AUDIO_ENDPOINT}?url=${encodeURIComponent(analyzedMetadata.originalLink)}`;
            const data = await fetchWithRetryAndTimeout(apiEndpoint, { headers: { 'accept': 'application/json' } });
...
function runSimulatedProgress(downloadUrl, type) {
    ...
    const interval = setInterval(() => {
        ...
        if (currentVal >= 100) {
            ...
            // Mengirim File Unduhan Langsung ke Browser
