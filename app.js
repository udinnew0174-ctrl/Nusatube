...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // BIND EVENT LISTENERS UNTUK OTO-SINKRONISASI KUALITAS
...
function triggerFeedback() {
    if (hapticEnabled && navigator.vibrate) {
        navigator.vibrate(12); // Efek getar halus seluler
    }
    if (soundEnabled) {
        try {
            ...
            // Browser memblokir AudioContext jika belum ada interaksi fisik
        }
    }
}
...
// FUNGSI UTAMA: Ambil Metadata dari Server API
    ...
    try {
        ...
        if (data && data.title) {
            analyzedMetadata = {
                ...
                videoDirectUrl: data.url, // Link Download Video
                originalLink: url,
                qualityGivenByAPI: data.quality || quality, // Menangkap resolusi asli dari JSON API
...
async function syncQualityOnDropdownChange() {
    if (!analyzedMetadata) return; // Jangan lakukan apa pun jika data analisis kosong
    ...
    // Memasang status loading mini pada tombol download video agar aman
    dlBtn.disabled = true;
    ...
    try {
        ...
        if (data && data.url) {
            // Perbarui URL download di memori dan Kualitas di DOM
        ...
        showToast(currentLang === 'id' ? "Kualitas tidak didukung oleh API, memakai cadangan terbaik." : "Quality not supported by API, keeping best fallback.", "error");
    } finally {
